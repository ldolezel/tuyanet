...
namespace com.clusterrr.TuyaNet
{
	...
	public class TuyaParser
	{
		...
		private TuyaLocalResponse ParseResponse(byte[] data)
		{
			...
			if (payload.Length == 0)
				return new TuyaLocalResponse(command, (int)returnCode, null,null, data);

			return new TuyaLocalResponse(command, (int)returnCode, payload,null, data);
		}
		...
		internal TuyaLocalResponse DecodeResponse34(byte[] data)
		{
			...
			return new TuyaLocalResponse(byteResponse.Command, byteResponse.ReturnCode, decodedBytes, json,data);
		}

		internal TuyaLocalResponse DecodeResponse(byte[] data)
		{
			return DecodeResponses(data)?.FirstOrDefault();
		}

		/// <summary>
		/// Decode one or more Tuya responses
		/// </summary>
		...
		public IEnumerable<TuyaLocalResponse> DecodeResponses(byte[] data, int index=0)
		{
         ...
         int payloadlen
			*/
			...
			if (arrlen<=0) yield break;

			if (version == TuyaProtocolVersion.V34)
				yield return DecodeResponse34(data.Skip(index).ToArray()); //TODO

			//todo rm next code and use decode34
			// Check length and prefix
			if (arrlen < 20 || !data.Skip(index).Take(PREFIX.Length).SequenceEqual(PREFIX))
			{
				throw new InvalidDataException("Invalid header/prefix");
			}
			...
			var onepacket = data.Skip(index).Take(ALLHEADLEN + length).ToArray();



			// Check suffix
			if (!onepacket.Skip(ALLHEADLEN + length - SUFFIX.Length).Take(SUFFIX.Length).SequenceEqual(SUFFIX))
			{
				throw new InvalidDataException("Invalid suffix");
			}
			...
			// Command
			var command = (TuyaCommand)BitConverter.ToUInt32(BigEndian(onepacket.Skip(CMDINDEX).Take(INTLEN)).ToArray(), 0);
			// Return code
			...
			{
				extractdata = extractdata.Skip(PROTOCOL_VERSION_BYTES_31.Length).ToArray();
				this.version = TuyaProtocolVersion.V31;
			}
			// Remove version 3.3 header
			if (extractdata.Take (PROTOCOL_VERSION_BYTES_33.Length).SequenceEqual(PROTOCOL_VERSION_BYTES_33))
			{
				extractdata = extractdata.Skip(PROTOCOL_33_HEADER.Length).ToArray();
			...
			{
				yield return new TuyaLocalResponse(command, returnCode, null, null, onepacket);
			}
			else
			{
			...
			//try another message in packet
			var rest = DecodeResponses(data,index + ALLHEADLEN + length)?.ToArray();
